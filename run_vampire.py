...
def print_metrics(results, label=""):
    ...
    macro_p, macro_r, macro_f, _ = precision_recall_fscore_support(expected, predicted, labels=labels, average='macro', zero_division=0)
    ...
    for l, p, r, f, s in zip(labels, precision, recall, f1, support):
        print(f"{l:<20} {p:>10.3f} {r:>10.3f} {f:>10.3f} {s:>10}")
    print(f"\n{'Macro Precision:':<20} {macro_p:.3f}")
...
# full corpus
print_metrics(results, label="Results — Full Corpus")

# successfully generated formulas only
solved = [r for r in results if r['predicted'] != 'error']
print_metrics(solved, label="Results — Successfully Generated Formulas")
